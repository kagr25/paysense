class ApiController < ApplicationController
  ...
  def check_content_type
    if request.method == 'POST' || request.method == 'PATCH'
      unless request.content_type.present? and (request.content_type.include?('application/json') || request.content_type.include?('text/csv') || request.content_type.include?('multipart/form-data'))
        render_api_error(04, 400, 'request', "Only content type application/json or text/csv is accepted.  Your content type: #{request.content_type}")
      end
    end
  end
